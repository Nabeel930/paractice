...
function checkno(noarray: number[]) {
  let count = 0;
...
let startingWithA: string[] = [];
function findA(words: string[]) {
...
console.log(result2);
//q13
let no0: number[] = [1, 2, 3, 4, 5, 6, 7];
let no1: number[] = [];
function squared(array: number[]) {
...
function check(arr: number[], no: number) {
  for (let i = 0; i < arr.length; i++) {
    if (no === arr[i]) {
      return i;
    }
...
function filterByLength(arr: string[], no: number) {
...
function countOccurences(arr: number[], no: number) {
  let count = 0;
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] === no) {
      count++;
...
function intoString(arr: string[]) {
...
function swapElements(arr: number[], i: number, z: number) {
  let empty = arr[i];
  arr[i] = arr[z];
  arr[z] = empty;
  return arr;
...
//q31
function noAppears(str: string, ch: string) {
  let count = 0;
...
function completedTask(arr: any[]) {
  for (let i = 0; i < arr.length; i++) {
...
let array20: number[] = [];
function reverseIt(array: number[]) {
...
//q35
function calculator(operande1: number, operande2: number, operator: any) {
  if (operator == "+") {
